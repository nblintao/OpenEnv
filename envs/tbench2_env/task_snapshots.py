...
Provider-agnostic core:
  ``_server_layer_commands(task_dir)``  shell commands that turn the official
      ...
      venv at ``/opt/envserver`` running THIS checkout's tbench2_env (source
      embedded into the build, so local fixes ship without a released
      package), plus the task directory staged at ``/opt/tb2-tasks/<id>``
...
Daytona materialization:
  ...
  bake CLI (``python -m tbench2_env.task_snapshots ...``)  optionally
      pre-register named snapshots ``<prefix><task-id>`` as a warm cache.
...
# Guard for the ONE payload that must be embedded into the build: this
# checkout's own env source (it exists nowhere downloadable until released).
...
_ENV_SRC_DIR = Path(__file__).resolve().parent
# What `pip install <dir>` needs from this checkout; everything else
# (uv.lock, caches, this file) stays out of the image.
_ENV_SRC_ITEMS = (
    "pyproject.toml",
    "README.md",
    "openenv.yaml",
...
def _dir_tar_b64(paths: list[Path], arcnames: list[str], max_bytes: int) -> str:
    ...
    if len(raw) > max_bytes:
        raise ValueError(
            f"embedded tar is {len(raw)} bytes (> {max_bytes}); "
            "inline embedding not suitable."
        )
...
def _resolve_docker_image(task_dir: Path, docker_image: str | None) -> str:
    ...
    if not docker_image:
        raise ValueError(
            f"{task_dir.name}: task.toml has no [environment].docker_image"
        )
    return docker_image
...
def build_task_image(task_dir: Path, docker_image: str | None = None):
    ...
    return (
        Image.base(base)
        .run_commands(*_server_layer_commands(task_dir))
...
def wait_server_ready(base_url: str, timeout_s: float = 300.0) -> None:
    ...
    while time.time() < deadline:
        ...
        time.sleep(2.0)
    raise TimeoutError(
        f"env server at {base_url} not ready in {timeout_s}s ({last_err})"
    )
...
def bake(daytona, tasks_dir: Path, task_id: str, prefix: str, force: bool) -> None:
    ...
    if existing is not None:
        if not force:
            print(
                f"[skip] {name} already exists (state={getattr(existing, 'state', '?')})"
            )
            return
    ...
    resources = task_resources(task_dir)
    print(
        f"[bake] {name}  cpu={resources.cpu} mem={resources.memory}G disk={resources.disk}G"
    )
...
def main() -> None:
    ...
    ap.add_argument(
        "--tasks-dir", required=True, help="local terminal-bench-2 checkout"
    )
    ...
    ap.add_argument(
        "--prefix", default="tb2-", help="snapshot name prefix (default: tb2-)"
    )
    ...
    if args.all:
        task_ids = sorted(
            p.name for p in tasks_dir.iterdir() if (p / "task.toml").is_file()
        )
    else:
